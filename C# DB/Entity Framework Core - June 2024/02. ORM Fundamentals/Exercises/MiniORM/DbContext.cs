...
namespace MiniORM
{
    public abstract class DbContext
    {
        ...
        {
            var dbSetProperties = this.GetType().GetProperties().Where(pi =>
                pi.PropertyType.IsGenericType && pi.PropertyType.GetGenericTypeDefinition() == typeof(DbSet<>));
        ...
        private IDictionary<Type, DbSetDescriptor> DescribeDbSets()
        {
            ...
            foreach (var (entityType, dbSetProperty) in this._dbSetProperties)
            {
                ...
                var dbSet = (IEnumerable<object>)dbSetProperty.GetValue(this);

                if (dbSet is not IEnumerable<object> typedDbSet)
                {
                    throw new InvalidOperationException($"DbSet {name} was not intialised correctly.");
                }
        ...
        private void MapAllRelations()
        {
            foreach (var dbSetDescriptor in this._dbSetDescriptors.Values)
            {
                var mapRelationsMethod = typeof(DbContext)
                    .GetMethod(nameof(MapAllRelations), BindingFlags.Instance | BindingFlags.NonPublic)!
            ...
            where T : class, new()
        {
            ...
            var entityType = typeof(T);
            var collectionProperties = entityType.GetProperties().Where(pi =>
            ...
            foreach (var property in collectionProperties)
            {
                ...
                var mapCollectionMethod = typeof(DbContext).GetMethod(nameof(MapCollection),
                    BindingFlags.Instance | BindingFlags.NonPublic)!.MakeGenericMethod(entityType, relatedEntityType);
            ...
            where T : class, new()
        {
            ...
            foreach (var foreignKeyProperty in entityType.GetProperties())
            {
                ...
                foreach (var entity in dbSet.Entities)
                {
                    ...
                    var relatedEntity = relatedDbSet.Single(re => Equals(relatedEntityKey.GetValue(re), foreignKey));
            ...
            where TRelated : class, new()
        {
            ...
            foreach (var entity in dbSet.Entities)
            {
                ...
                var relatedEntities = relatedDbSet.Where(re => Equals(
                    foreignKeyProperty.GetValue(re), primaryKey)).Cast<TRelated>().ToArray();
